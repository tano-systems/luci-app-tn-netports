...
function NetPorts(inputConfig) {
	...
	var createBase = function() {
		...
		var title = E('div', { class: 'netports-title' }, [
			E('h3', {}, _('Network Interfaces Ports Status')),
			E('div', { class: 'netports-title-v' }, [
				E('div', { class: 'netports-copyright' },
					E('a', { href: 'https://github.com/tano-systems/luci-app-netports' },
						'luci-app-netports ' + version)
				),
		...
		targetElement.appendChild(
			E('div', { class: 'cbi-section' }, [ title, tableWrapper ])
		);
	...
	var init = function(inputConfig) {
		...
		config.mode = inputConfig.mode !== undefined ? inputConfig.mode : config.mode;
		...
		if (config.mode == NetPortsMode.V) {
			data.forEach(function(port) {
				...
				if (rows[n]) {
					 ...
					 */
					cells_orig = rows[n].querySelectorAll('.td');

					for (var cn = 0; cn < cells_orig.length; cn++) {
